...
export default function Signup(params) {
        ...
        const ProceedRegister = (e) => {
          e.preventDefault();
          if (validate()) {
            const url = `http://178.62.5.150:1944/api/register?name=${username}&email=${useremail}&password=${pass}&password_confirmation=${com_pass}`
        ...
        const setDefaultPreference = (id) =>{
          const url = `http://178.62.5.150:1944/api/post_preference?user_id=${id}&authors=king&sources=bbc,cnn`
          const requestOptions = {
            method: 'POST',
          }
